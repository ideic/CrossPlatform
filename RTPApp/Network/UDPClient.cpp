...
void UDPClient::Init()
{
    ...
    if (auto res = setsockopt(socketInfo_->socketId, SOL_SOCKET, SO_SNDBUF, (char*)&MAX_MSG_SIZE, sizeof(MAX_MSG_SIZE)); res == SOCKET_ERROR)
    {
        auto error = MY_GET_LAST_ERROR;
...
bool UDPClient::SendData(std::string_view message)
{
    ...
    if (auto res = sendto(socketInfo_->socketId, message.data(), static_cast<int>(message.size()), 0, reinterpret_cast<sockaddr*>(&servaddr), sizeof(servaddr)); res == SOCKET_ERROR) {
		auto error = MY_GET_LAST_ERROR;
